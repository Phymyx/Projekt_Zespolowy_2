...
$snmp_name = "UpsOutputVoltage";
$snmpId1 = '1.3.6.1.2.1.33.1.4.4.1.2.1';
$snmpId2 = '1.3.6.1.2.1.33.1.4.4.1.2.2';
$snmpId3 = '1.3.6.1.2.1.33.1.4.4.1.2.3';
...
require_once dirname(__FILE__) . '/../db/db_config.php';
...
function save_snmp($snmpName, $snmpId, $db_table_name) {
	...
	$db = connect();
	$result = mysqli_query($db, "INSERT INTO $db_table_name(snmp_oid, name, value) 
		VALUES('$snmpId','$snmpName', '$intValue')");
	if ($result) {
		echo'<br>';
		echo "Dodane do bazy";
		echo'<br>';
...
function retrieve_snmp_by_date($snmpName, $snmpId, $db_table_name, $date_lower, $date_upper, 
	$start_from=0, $limit=10, $desc=True) {
	$db = connect();
	//$result = mysqli_query($db, "SELECT * FROM $db_table_name");
	$desc = ($desc== FALSE) ? "ASC" : "DESC";
...
function retrieve_snmp($snmpName, $snmpId, $db_table_name, $start_from=0, $limit=10, $desc=True) {
	$db = connect();
	$desc = ($desc== FALSE) ? "ASC" : "DESC";
	$limit = ($limit <0) ? 1 : $limit;
	$result = mysqli_query($db, "SELECT * FROM $db_table_name ORDER BY id $desc LIMIT $start_from, $limit");
